...
from precompute_word_vector_nearest_neighbors import precompute_word_vector_nearest_neighbors
manager = Manager(app)
...
def populate_tables():
    ...
    precompute_word_vector_nearest_neighbors()
    print('Finished precomputing nearest neighbors in {:.0f} seconds'.format(time()-start))
    start = time()
    # Populate the Word table, and commit the changes
    populate_word_table(Word, db)
    print('Finished populating Word table in {:.0f} seconds'.format(time()-start))
    ...
    print('Finished populating SubgraphemeFrequency table in {:.0f} seconds'.format(time()-start))

    start = time()
    # Populate the SubphonemeFrequency table, and commit the changes
    populate_subphoneme_frequency_table(SubphonemeFrequency, db)
    print('Finished populating SubphonemeFrequency table in {:.0f} seconds'.format(time()-start))

    start = time()
    # Populate the FasttextNeighbor table, and commit the changes
    populate_fasttext_neighbor_table(FasttextNeighbor, db)
    print('Finished populating FasttextNeighbor table in {:.0f} seconds'.format(time()-start))
